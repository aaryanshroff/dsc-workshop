...
async function start() {
  ...
  k.add([k.sprite("map_sprite"), k.pos(0, 0), k.z(0), k.anchor("topleft")]);
  ...
  function canMoveTo(x, y) {
    const tL = isWalkable(
      Math.floor((x - HITBOX_RADIUS) / TILE_SIZE),
      Math.floor((y - HITBOX_RADIUS) / TILE_SIZE),
    );
    const tR = isWalkable(
      Math.floor((x + HITBOX_RADIUS) / TILE_SIZE),
      Math.floor((y - HITBOX_RADIUS) / TILE_SIZE),
  ...
  outer: for (let radius = 0; radius < Math.max(MAP_W, MAP_H); radius++) {
  ...
  k.onUpdate(() => {
    ...
    if (
      ...
      k.get("enemy").length < MAX_ENEMIES
    ) {
      spawnEnemy();
      spawnTimer = 0;
    }
  ...
  k.onUpdate(() => {
    ...
    if (moveDir !== 0) {
      ...
      const speed =
        PLAYER_SPEED * getSpeedMultiplier(player.pos.x, player.pos.y);
  ...
  function drawHealthBar(entity) {
    ...
    k.drawRect({
      ...
      width: barW * ratio,
      height: barH,
      color: ratio > 0.5 ? k.rgb(0, 200, 0) : k.rgb(200, 0, 0),
    });
  }
  ...
  k.onDraw(() => {
    ...
    const topLeft = k.vec2(
      cam.x - k.width() / 2 / camScale.x,
      cam.y - k.height() / 2 / camScale.y,
    );
    k.drawText({
      text: `Score: ${score}`,
